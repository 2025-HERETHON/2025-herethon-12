...
function renderPreview(photos) {
  ...
  // 기존 사진 개수는 일단 카운트에서 제외 (이미 템플릿을 통해 미리보기 됨)

  photos.forEach((photo, i) => {
    const targetIndex = currentCount + i;
    if (targetIndex >= previews.length) return;

    const reader = new FileReader();
    reader.onload = (e) => {
      const previewBox = previews[targetIndex];
...
mainForm.addEventListener("submit", (e) => {
  ...
  // 현재 올린 사진이 아니라, 해당 게시물에 총 몇 장의 사진이 올라와있는지로 확인
